...
Same verified corrections as docs/REPORT_OUTLINE.md apply here: STREAM is 990 cycles not
...
**Show:**
- Figure: docs/edge_demo_sobel_x.png - input cat on the left, RTL hardware output on the right; the whole story in one image.
...
**Show:**
- Diagram to draw: 5-box block diagram with en / edge flags[4] / coef[9] / win_valid / busy arrows - see docs/BLOCK_DIAGRAM_NOTES.md (the ASCII sketch in the "Block diagram" section).
...
**Show:**
- Figure: docs/waveforms/Window_generator_producing_valid_taps.PNG - unit test, 3x3 window sliding across the taps, first valid windows.
- Figure: docs/waveforms/edges.PNG - output col 31 then col 0: right_edge masks c,f,i then left_edge masks a,d,g, centre untouched.
...
**Show:**
- Diagram to draw: detailed datapath with the three dashed stage boundaries, valid chain level with the registers, "sat (top-bits check)", RELU toggle, LB0/LB1 SRL annotation, latency 37 - see docs/BLOCK_DIAGRAM_NOTES.md.
- Figure: docs/waveforms/throughput.PNG - mid frame, valid_out solid high, pixel_out changes every cycle.
...
**Show:**
- Diagram to draw: FSM state diagram with per-state outputs (en, win_valid, edge flags registered, busy, frame_done) - see docs/BLOCK_DIAGRAM_NOTES.md.
- Figure: docs/waveforms/stall.PNG - the 3-cycle stall before pixel 300 and its bubble.
- Figure: docs/waveforms/drain.PNG (optional, if space) - DRAIN, 34 more outputs, frame_done, busy falling 3 cycles early.
...
- Figure: docs/edge_demo_sobel_y.png or docs/edge_demo_laplacian.png - RTL output rendered from hw_out/, identical to golden PNG.
...
- Figure: docs/waveforms/latency.PNG - first valid_in at 145 ns to first valid_out at 515 ns = 370 ns = 37 cycles.
...
**Show:**
- Table: Iterations - columns Change | LUTs | WNS | Fmax | Power | FoM | Outcome - rows baseline, in_cnt removal (reverted), registered flags, top-bits sat, SAIF power - source docs/DESIGN_ITERATIONS.md.
...
**Show:**
- Figure: docs/waveforms/fsm.PNG - the whole frame IDLE-FILL-STREAM-DRAIN-DONE with in_cnt/drain_cnt, as the closing picture.
- Figure: docs/waveforms/kernel_load.PNG (small inset) - 9 write_en pulses, write_addr 0..8, busy low: the programmable-kernel bonus in one shot.
...
**Sources:** fpga/reports/RESULTS.md, fpga/reports/utilization.rpt, fpga/reports/timing_summary.rpt, fpga/reports/paths.rpt, fpga/reports/power.rpt, docs/DESIGN_ITERATIONS.md, docs/POWER_METHODOLOGY.md, docs/BLOCK_DIAGRAM_NOTES.md, docs/WAVEFORM_CAPTURES.md, tb/tb_top_all.sv, golden_model/golden.py.
